...
export default {
  ...
  mounted() {
    ...
    let getApi = [
      ...
      getFourModual({ timeid: this.initTime }),
    ];
    /**
     * 响应数据处理队列
     * */
    let resApi = [this.requestAllTimes, this.requestFourModulesData];
    //请求组件所需要数据
  ...
  methods: {
    ...
    async getTimeINfo() {
      ...
      let indexData = await getFourModual({ timeid: this.initTime });
      this.requestAllTimes(indexData.data);
    },
